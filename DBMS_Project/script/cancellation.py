...
con = engine.connect()
def writetocsv(record, file_name="missingcountylogs.csv"):
...
for index, row in df.iterrows():
    ...
    cancelled = row['CANCELLED'] if not (pd.isnull(row['CANCELLED'])) else 0
    if pd.isnull(crs):
        crs = 0
    ...
    con.execute(
        ...
        "FK_CANCELLATION_ID, FK_DATE_ID, CRS_DEP_TIME, DEP_TIME_BLK, WHEELS_ON, WHEELS_OFF, DEP_DELAY, DEP_TIME, CANCELLED) "
